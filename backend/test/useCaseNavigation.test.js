...
test('golf use case includes a dedicated golf accessories filter', () => {
  ...
  assert.match(golfConfig, /\['Wearables',\['scarf','scarves','hat','hats','cap','caps'/);
  assert.match(golfConfig, /'wearable','wearables','apparel'\]\]/);
  assert.match(golfConfig, /\['Bags',\['bag','bags','golf bag','golf bags'/);
  assert.match(golfConfig, /\['Drinkware',\['drinkware','water bottle','water bottles','tumbler','tumblers'/);
...
test('travel product types use clear journey-oriented groups', () => {
  ...
  for (const label of ['Luggage, Travel Bags & Backpacks', 'Coolers & Lunch Bags', 'Travel Drinkware', 'Blankets, Towels & Travel Pillows', 'Packing & Travel Accessories', 'Camping, Picnic & Outdoor Gear', 'Mascots & Plush', 'Tech & Charging']) {
